...
uint32 ReadBuild(int locale)
{
    ...
    std::string filename = Kitron::StringFormat("component.wow-%s.txt", langs[locale]);
...
void ExtractMapsFromMpq(uint32 build)
{
    ...
    for(uint32 z = 0; z < map_count; ++z)
    {
        ...
        mpqMapName = Kitron::StringFormat("World\\Maps\\%s\\%s.wdt", map_ids[z].name, map_ids[z].name);
        WDT_file wdt;
...
//            printf("Error loading %s map wdt data\n", map_ids[z].name);
        ...
        for(uint32 y = 0; y < WDT_MAP_SIZE; ++y)
        {
            for(uint32 x = 0; x < WDT_MAP_SIZE; ++x)
            {
                ...
                mpqFileName = Kitron::StringFormat("World\\Maps\\%s\\%s_%u_%u.adt", map_ids[z].name, map_ids[z].name, x, y);
                outputFileName = Kitron::StringFormat("%s/maps/%03u%02u%02u.map", output_path, map_ids[z].id, y, x);
...
{
    std::string fileName = Kitron::StringFormat("%s/Data/%s/locale-%s.MPQ", input_path, langs[locale], langs[locale]);
    ...
    for(int i = 1; i < 5; ++i)
    {
        std::string ext;
        if (i > 1)
            ext = Kitron::StringFormat("-%i", i);

        fileName = Kitron::StringFormat("%s/Data/%s/patch-%s%s.MPQ", input_path, langs[locale], langs[locale], ext.c_str());
        if (boost::filesystem::exists(fileName))
            new MPQArchive(fileName.c_str());
    }
}
...
void LoadCommonMPQFiles()
{
    ...
    {
        fileName = Kitron::StringFormat("%s/Data/%s", input_path, CONF_mpq_list[i]);
        if (boost::filesystem::exists(fileName))
            new MPQArchive(fileName.c_str());
    }
}
...
int main(int argc, char * arg[])
{
    ...
    {
        std::string filename = Kitron::StringFormat("%s/Data/%s/locale-%s.MPQ", input_path, langs[i], langs[i]);
        if (boost::filesystem::exists(filename))
        {
