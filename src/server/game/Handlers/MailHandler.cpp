...
void WorldSession::HandleSendMail(WorldPackets::Mail::SendMail& sendMail)
{
    ...
    {
        SendNotification(GetKitronString(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    auto mailCountCheckContinuation = [this, player = _player, receiverGuid, mailInfo = std::move(sendMail.Info), reqmoney, cost](uint32 receiverTeam, uint64 mailsCount, uint8 receiverLevel, uint32 receiverAccountId) mutable
    {
        ...
        {
            SendNotification(GetKitronString(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
            return;
        }
        ...
        if (!mailInfo.Attachments.empty() || mailInfo.SendMoney > 0)
        {
            ...
            if (!mailInfo.Attachments.empty())
            {
                for (Item* item : items)
                {
                    if (log)
                    {
                        sLog->outCommand("GM %s (GUID: %u) (Account: %u) mail item: %s (Entry: %u Count: %u) "
            ...
            {
                sLog->outCommand("GM %s (GUID: %u) (Account: %u) mail money: %u to: %s (%s) (Account: %u)",
            ...
            if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sCharacterCache->GetCharacterNameByGuid(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetKitronStringForDBCLocale(LANG_UNKNOWN);
                }
                sLog->outCommand("GM %s (Account: %u) receiver mail item: %s (Entry: %u Count: %u) and send COD money: %u to player: %s (Account: %u)",
