...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (!factionTemplate)
    {
        TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) has non-existing faction template (%u). This can lead to crashes, aborting.", cInfo->Entry, cInfo->faction);
        ABORT();
    }
...
bool ObjectMgr::LoadKitronStrings()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 Kitron strings. DB table `trinity_string` is empty. You have imported an incorrect database for more info search for TCE00003 on forum.");
        return false;
    }
