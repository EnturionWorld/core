...
public:
    ...
    */
    ...
    static bool HandleCharacterTitlesCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
    {
        ...
        char const* knownStr = handler->GetKitronString(LANG_KNOWN);
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            if (titleInfo && target->HasTitle(titleInfo))
            {
                ...
                if (target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->MaskID)
                    activeStr = handler->GetKitronString(LANG_ACTIVE);

                std::string titleName = Kitron::StringFormat(name, player->GetName().c_str());
        ...
        if (newNameV)
        {
            ...
            if (WorldSession* session = handler->GetSession())
            {
                if (Player* player = session->GetPlayer())
                    sLog->outCommand("GM %s (Account: %u) forced rename %s to player %s (Account: %u)", player->GetName().c_str(), session->GetAccountId(), newName.c_str(), player->GetName().c_str(), sCharacterCache->GetCharacterAccountIdByGuid(player->GetGUID()));
            }
            else
                sLog->outCommand("CONSOLE forced rename '%s' to '%s' (%s)", player->GetName().c_str(), newName.c_str(), player->GetGUID().ToString().c_str());
        }
        else
        {
            if (Player* target = player->GetConnectedPlayer())
    ...
    static bool HandleCharacterChangeAccountCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, AccountIdentifier newAccount)
    {
        ...
        std::string logString = Kitron::StringFormat("changed ownership of player %s (%s) from account %u to account %u", player->GetName().c_str(), player->GetGUID().ToString().c_str(), oldAccountId, newAccount.GetID());
        if (WorldSession* session = handler->GetSession())
        {
            if (Player* player = session->GetPlayer())
                sLog->outCommand("GM %s (Account: %u) %s", player->GetName().c_str(), session->GetAccountId(), logString.c_str());
        }
        else
            sLog->outCommand("%s %s", handler->GetKitronString(LANG_CONSOLE), logString.c_str());
        return true;
    }
    ...
    static bool HandleCharacterReputationCommand(ChatHandler* handler, Optional<PlayerIdentifier> player)
    {
        ...
        for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
        {
            ...
            std::string rankName = handler->GetKitronString(ReputationRankStrIndex[rank]);
